...
import { calcArriendoResult, formatUF, formatCLP, formatPercent } from '@/lib/evaluacion/engines';
import { STATE_LABELS, MATERIAL_LABELS } from '@/lib/evaluacion/types';
import type { ValuationResult, MarketParams, Subject, ArriendoAnalysis, CasaValuationResult, DeptoValuationResult } from '@/lib/evaluacion/types';
import { generatePDF } from '@/lib/evaluacion/pdfExport';
...
export function ReportStep({ result, params, subject, arriendoAnalysis, onBack, onReset }: ReportStepProps) {
  ...
  const casa = result.property_type === 'CASA' ? (subject as Partial<import('@/lib/evaluacion/types').SubjectCasa>) : null;
  const depto = result.property_type === 'DEPTO' ? (subject as Partial<import('@/lib/evaluacion/types').SubjectDepto>) : null;
